 ...
 */
export async function projectsDeleteProjectApiKey(
  client: ArgotOpenAiCore,
  request: operations.DeleteProjectApiKeyRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.ProjectApiKeyDeleteResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.DeleteProjectApiKeyRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ProjectApiKeyDeleteResponse$inboundSchema),
