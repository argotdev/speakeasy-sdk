 ...
 */
export async function projectsRetrieveProjectServiceAccount(
  client: ArgotOpenAiCore,
  request: operations.RetrieveProjectServiceAccountRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.ProjectServiceAccount,
    | SDKError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.RetrieveProjectServiceAccountRequest$outboundSchema.parse(
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ProjectServiceAccount$inboundSchema),
    M.fail(["4XX", "5XX"]),
  )(response);
  if (!result.ok) {
    return result;
