 ...
 */
export async function audioCreateTranslation(
  client: ArgotOpenAiCore,
  request: components.CreateTranslationRequest,
  options?: RequestOptions,
): Promise<
  Result<
    operations.CreateTranslationResponseBody,
    | SDKError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) => components.CreateTranslationRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.CreateTranslationResponseBody$inboundSchema),
    M.fail(["4XX", "5XX"]),
  )(response);
  if (!result.ok) {
    return result;
